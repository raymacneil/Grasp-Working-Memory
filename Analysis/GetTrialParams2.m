function TrialParamsOut = GetTrialParams2(datadir)
...
for ii = 1:numel(IDs)
    ...
    for typ = 1:3
        ...
        if typ < 3
            ...
            % Cross reference OTCReogranizedTF with OTCReorganized to
            % determine additional timeouts.
            TargFolders = ["OTCReorganizedTF"; "OTCReorganized"];
            TempCell = cell(numel(TargFolders),1);

            for kk = 1:numel(TargFolders)
                
                TargKK = TargFolders(kk);
                OptkData = fullfile(dirLook, dirContents(tarIdx), TargKK);

                if size(OptkData,1) > 1
                    OptkData = strip(OptkData(1,:));
                end
                allFiles = dir(OptkData);
                fileNames = {allFiles.name};
                fileNames = regexp(fileNames,'\w*\.txt', 'match');
                fileNames = vertcat(fileNames{:});
                
                if strcmp(TargKK, "OTCReorganizedTF")
                   exp = 'tf(?=.txt)';
                   NewFileNames = regexprep(fileNames,exp,'');
                   fileNames = NewFileNames;
                end
                
                % fName(end-5:end-4) = []
                TempCell(kk) = {fileNames};                
            end
            
            OTCtf = TempCell{1};
            OTC = TempCell{2};
            discarded = setdiff(OTC,OTCtf);
            
            if ~isempty(discarded)
                myfun = @(x) strsplit(x,'_'); 
                PutBackOrganized = cellfun(myfun, discarded, 'UniformOutput', false);
                PutBackOrganized = vertcat(PutBackOrganized{:});
                PutBackOrganized = sortrows(PutBackOrganized,[3,5,4,6,7]);
                              
                if typ == 1
                   idxRetain = ~strcmp(PutBackOrganized(:,3), 'bn00');                
                elseif typ == 2
                   idxRetain = ~or(strcmp(PutBackOrganized(:,3), {'bn00'}),...
                       strcmp(PutBackOrganized(:,3), {'bn04'}));
                end
                
                PutBackOrganized = PutBackOrganized(idxRetain,1:7);
                PutBackNumerics = cellfun(@(x) str2num(x),... 
                    regexprep(PutBackOrganized(:,3:end), '\D*', '')); %#ok<ST2NM>
                [uB, ~, Groups] = unique(PutBackNumerics(:,1));
                BlockedData = splitapply(@(x){x}, PutBackNumerics, Groups);
                
                
                for ll = 1:numel(BlockedData)
                    llBlock = BlockedData{ll};
                    numNewRows = size(llBlock,1);
                    Tadd = cell(numNewRows,size(T,2));
                    Tadd(:,[1:4,20]) = cellstr(repmat(T{1,[1:4,20]},numNewRows,1));
                    Tadd(:,28:31) = num2cell(repmat(T{1,28:end}, numNewRows,1));
                    rowIdxBlockNum = T.BlockNum == uB(ll);
                    rowIdxBlockNumFirst = find(rowIdxBlockNum,1, 'first');
                    colIdxBlockNum = strcmp(T.Properties.VariableNames, "BlockNum");
                    colIdxTrialCount = strcmp(T.Properties.VariableNames, "TrialCount");
                    colIdxTrialNum = strcmp(T.Properties.VariableNames, "TrialNum");
                    colIdxBadTrial = strcmp(T.Properties.VariableNames, "BadTrial");
                    colIdxRptTrial = strcmp(T.Properties.VariableNames, "TrialRepeat");
                    colIdxBarLength = strcmp(T.Properties.VariableNames, "BarLength");
                    Tadd(:,colIdxTrialCount) = num2cell(1:1:numNewRows);
                    TNumSt = numNewRows + 1;
                    TNumEn = numNewRows + nnz(rowIdxBlockNum);
                    T(rowIdxBlockNum, colIdxTrialCount) = num2cell(TNumSt:1:TNumEn)'; 
                    Tadd(:,or(colIdxBlockNum,colIdxTrialNum)) = [num2cell(repelem((uB(ll)),numNewRows,1)),... 
                        num2cell(llBlock(:,2))];
                    Tadd(:,or(colIdxBadTrial,colIdxRptTrial)) = [num2cell(llBlock(:,5)),...
                        num2cell(llBlock(:,4))];
                    Tadd(:,[7,8,12]) = num2cell(repelem(0,numNewRows,3));
                    Tadd(:,colIdxBarLength) = num2cell(repelem(T{rowIdxBlockNumFirst,colIdxBarLength},numNewRows,1));
                    NaNFillIdx = [14:19,21,22,25:27];
                    Tadd(:,NaNFillIdx) = num2cell(repelem(NaN,numNewRows,numel(NaNFillIdx)));
                    Tadd(:,[23,24]) = num2cell(repelem("NA",numNewRows,2));
                    Tadd = cell2table(Tadd,"VariableNames",T.Properties.VariableNames);
                    T = [T(1:(rowIdxBlockNumFirst-1),:); Tadd;...
                            T(rowIdxBlockNumFirst:end,:)];
