...
% data together.
    ...
    varNames = ["id","Block","Trial_id","TrialCount_tfsdat",...
                ...
                "LDJ_Early","SPARC_Early", "ft0","fRonH","fRonTh","fRonI","fRon","fPGA","fRoffH","fRoffTh","fRoffI","fRoff","fAdjusted_Roff","Adj_Roff_markerFlag",...
...
%             end
            ...
            T.fRoffI(ii) = fwdReachEnd1(temp.mkrIXYZ_vel, T.fPGA(ii), VTon, VToff, VDon, VDoff);
    
            % Thumb info
            T.fRonTh(ii) = fwdReachStart(temp.mkrTXYZ_vel, VTon, VToff, VDon, VDoff);
            T.fRoffTh(ii) = fwdReachEnd1(temp.mkrTXYZ_vel, T.fPGA(ii), VTon, VToff, VDon, VDoff);
            
            % Hand info
            T.fRonH(ii) = fwdReachStart(temp.mkrHXYZ_vel, VTon, VToff, VDon, VDoff);
            T.fRoffH(ii) = fwdReachEnd1(temp.mkrHXYZ_vel, T.fPGA(ii), VTon, VToff, VDon, VDoff);
            
            % Final values
            T.fRon(ii) = min(T.fRonI(ii),T.fRonTh(ii));
            T.fRoff(ii) = min(T.fRoffI(ii),T.fRoffTh(ii));
            [T.fAdjusted_Roff(ii), T.Adj_Roff_markerFlag(ii)]  = fwdReachEnd2(temp, VTon, VToff, VDon, VDoff);
            ...
            if any(ismember(T.fPGA(ii):T.fRoff(ii),interpolation_flag2_ref))
                T.GAClose_Interp_Flag(ii) = 2;
                T.GAClose_Knot_Size(ii) = sum(ismember(T.fPGA(ii):T.fRoff(ii),interpolation_flag2_ref));
            elseif any(ismember(T.fPGA(ii):T.fRoff(ii),interpolation_flag1_ref))
                T.GAClose_Interp_Flag(ii) = 1;
                T.GAClose_Knot_Size(ii) = sum(ismember(T.fPGA(ii):T.fRoff(ii),interpolation_flag1_ref));
            else
